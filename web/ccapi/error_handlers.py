...
def error(code, *args, **kwargs):
    ...
    if code == 403: return(four_oh_three(*(args), **(kwargs)))
...
def four_oh_three(*args, **kwargs):
    ...
    error = {"code": 403, "type": "unauthorized", "message": args, "details": kwargs}
...
# If a generic 500 is called, return a 500 with as little processing as possible since it shouldn't
# ever come to this
@app.errorhandler(500)
def error_on_error():
    return "{\"code\": 500, \"message\": [ \"INTERNAL SERVER ERROR\" ]}", 500

# When the framework throws a 404 this will handle it, different than the custom 404 handler because
# it doesn't take arguments, and relies on access to the flask request object
@app.errorhandler(404)
def not_found(e):
    error = {"code": 404, "type": "resource not found", "message": "unknown location '%s'" %(flask.request.full_path)}
    return json.dumps(error), 404